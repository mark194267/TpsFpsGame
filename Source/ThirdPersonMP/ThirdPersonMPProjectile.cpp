...
AThirdPersonMPProjectile::AThirdPersonMPProjectile()
{
	...
	// Call Replicates to sym with server
	bReplicates = true;
	...
	if (DefaultMesh.Succeeded())
	{
		...
		StaticMesh->RelativeScale3D = FVector(0.15f, 0.15f, 0.15f);
	}
...
{
	FString healthMessage = FString::Printf(TEXT("You hit something."));

	// Don`t hit owner;
	if (OtherActor != GetOwner())
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, healthMessage);
		UGameplayStatics::ApplyPointDamage(OtherActor, Damage, NormalImpulse, Hit, Instigator->Controller, this->GetOwner(), DamageType);
		Destroy();
	}
}
